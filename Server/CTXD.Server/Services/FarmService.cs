...
public sealed record FarmStartAllRequest(int Type);
public sealed record FarmStartResult(int GeneralId,int Type,int Food,int Reward,DateTimeOffset EndsAt);
public sealed record FarmStartAllResult(int Type,int Food,FarmStartResult[] Items);
    ...
    GamePushHub push)
{
    ...
    public async Task<FarmStartAllResult> StartAllAsync(long playerId,int type,CancellationToken ct)
    {
        if(type is<0 or>3)throw new GameException("FARM_TYPE_INVALID","Farm type must be between 0 and 3.");
        await using var c=await db.DataSource.OpenConnectionAsync(ct);await using var t=await c.BeginTransactionAsync(ct);var p=await PlayerAsync(c,t,playerId,true,ct);RequireOpen(p.Level);await EnsureAsync(c,t,playerId,p.Force,ct);var city=FarmCity(p.Force);var desired=FarmIdleState+1+type;
        int farmLevel;await using(var q=new NpgsqlCommand("SELECT level FROM nation_farm_state WHERE force_id=$1",c,t)){q.Parameters.AddWithValue(p.Force);farmLevel=Convert.ToInt32(await q.ExecuteScalarAsync(ct));}var def=Farm(farmLevel);var coe=Coefficient(p.Level);var foodPer=type switch{2=>def.ConsumeFood*coe,3=>def.ConsumeFood2*coe,_=>0};var extra=type switch{2=>def.ExpExtra,3=>def.ExpExtra2,_=>0};var bonus=type==0?0:await CityExpBonusAsync(c,t,p.Force,ct);var reward=((type==0?def.FoodReward:def.ExpReward)+extra)*coe+bonus;var minutes=type==0?def.FoodTime:def.ExpTime;
        var candidates=new List<(int GeneralId,bool NeedsToken)>();await using(var q=new NpgsqlCommand(@"SELECT g.general_id,g.state,f.type FROM player_generals g LEFT JOIN player_farms f ON f.player_id=g.player_id AND f.general_id=g.general_id WHERE g.player_id=$1 AND g.general_type=2 AND g.location_id=$2 AND g.state>=$3 AND g.state<>$4 ORDER BY g.general_id FOR UPDATE OF g",c,t)){q.Parameters.AddWithValue(playerId);q.Parameters.AddWithValue(city);q.Parameters.AddWithValue((short)FarmIdleState);q.Parameters.AddWithValue((short)desired);await using var r=await q.ExecuteReaderAsync(ct);while(await r.ReadAsync(ct)){var existingType=r.IsDBNull(2)?(int?)null:r.GetInt16(2);if(existingType==type)continue;candidates.Add((r.GetInt32(0),existingType is null));}}
        if(candidates.Count==0)throw new GameException("FARM_NO_GENERAL","No general can start this Farm work.",409);var tokenCount=candidates.Count(x=>x.NeedsToken);if(tokenCount>0&&await ItemCountAsync(c,t,playerId,ct)<tokenCount)throw new GameException("FARM_TOKEN_NOT_ENOUGH","Not enough Farm tokens 1701.",409);
        var totalFood=foodPer*candidates.Count;if(totalFood>0){await production.AccrueAndGetAsync(playerId,ct,c,t);await using var pay=new NpgsqlCommand("UPDATE player_resources SET food=food-$2 WHERE player_id=$1 AND food>=$2",c,t);pay.Parameters.AddWithValue(playerId);pay.Parameters.AddWithValue(totalFood);if(await pay.ExecuteNonQueryAsync(ct)!=1)throw new GameException("FARM_FOOD_NOT_ENOUGH","Not enough food for Farm training.",409);}
        var now=DateTimeOffset.UtcNow;var end=now.AddMinutes(minutes);foreach(var candidate in candidates){await using(var save=new NpgsqlCommand(@"INSERT INTO player_farms(player_id,general_id,type,started_at,ends_at,reward,duration_minutes) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(player_id,general_id) DO UPDATE SET type=$3,started_at=$4,ends_at=$5,reward=$6,duration_minutes=$7",c,t)){save.Parameters.AddWithValue(playerId);save.Parameters.AddWithValue(candidate.GeneralId);save.Parameters.AddWithValue((short)type);save.Parameters.AddWithValue(now);save.Parameters.AddWithValue(end);save.Parameters.AddWithValue(reward);save.Parameters.AddWithValue(minutes);await save.ExecuteNonQueryAsync(ct);}await SetGeneralStateAsync(c,t,playerId,candidate.GeneralId,desired,ct);}if(tokenCount>0&&!await items.ConsumeAsync(c,t,playerId,FarmTokenItem,FarmTokenType,tokenCount,ct))throw new GameException("FARM_TOKEN_NOT_ENOUGH","Not enough Farm tokens 1701.",409);
        var result=new FarmStartAllResult(type,totalFood,candidates.Select(x=>new FarmStartResult(x.GeneralId,type,foodPer,reward,end)).ToArray());await t.CommitAsync(ct);await push.SendAsync(playerId,"farm.updated",new{kind="startAll",type,count=result.Items.Length,food=totalFood},ct);return result;
    }
    ...
    async Task<FarmStartResult> StartCoreAsync(NpgsqlConnection c,NpgsqlTransaction t,long playerId,int generalId,int type,bool autoEnter,CancellationToken ct)
    {
        ...
        var switching=!autoEnter&&state!=desired&&state>FarmIdleState;if(!autoEnter&&state==desired)throw new GameException("FARM_GENERAL_BUSY","General is already doing this Farm work.",409);if(!autoEnter&&!switching&&state!=FarmIdleState)throw new GameException("FARM_GENERAL_BUSY","General cannot start Farm work in the current state.",409);
