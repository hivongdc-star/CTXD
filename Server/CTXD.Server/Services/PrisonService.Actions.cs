...
public sealed partial class PrisonService
{
    public async Task<PrisonView> GetAsync(long playerId,CancellationToken ct)
    {
        ...
        if(holder is null)
        {
            ...
            return new(false,have,0,false,false,0,data.Degrees.Count,0,0,0,0,5,0,false,0,0,0,Degrees(),[],captives);
        }
        ...
        var degree=data.Degrees[holder.LashLv];
        ...
        return new(true,havePic,holder.PrisonLv,canUpdate,havePic,holder.LashLv,data.Degrees.Count,degree.ExpExtra,degree.TimeExtra,upgradeGold,holder.GrabNum,quality,holder.AutoLashExp,haveTech,holder.Point,degree.ExpFree,degree.ExpSum,Degrees(),prisoners,captives);
    }
    ...
    public async Task<PrisonLashResult> LashAsync(long playerId,long slaveId,CancellationToken ct)
    {
        ...
        if(escapeAt.HasValue&&holder.LashLv>=2){added=degree.TimeExtra;nextEscape=escapeAt.Value.AddSeconds(added);}
        ...
        return new(slaveId,reward,added,holder.LashLv,point);
    }
    ...
    public async Task<PrisonEscapeResult> EscapeAsync(long playerId,int generalId,CancellationToken ct)
    {
        ...
        var extra=0;if(slashTimes>0){var holder=await ReadHolderAsync(c,t,holderPlayer,false,ct);if(holder is not null)extra=data.Degrees[holder.LashLv].TimeExtra;}
