...
public sealed class QuestService(GameDb db,CanonicalContent content,ExperienceService experience,ResourceProductionService production)
{
    ...
    static int[] Args(TargetDefinition target)=>target.Args.Select(Int).ToArray();
    async Task<(bool complete,string? dependency)> EvaluateAsync(NpgsqlConnection c,NpgsqlTransaction? t,long player,TaskDefinition task,CancellationToken ct)
    {
        switch(task.Target.Kind)
        {
            ...
            case "equip":
                var equip=Args(task.Target);await using(var cmd=new NpgsqlCommand("SELECT COALESCE(sum(num),0)>=$5 FROM player_equipment WHERE player_id=$1 AND goods_type=$2 AND quality>=$3 AND level>=$4",c,t)){cmd.Parameters.AddWithValue(player);cmd.Parameters.AddWithValue(equip[0]);cmd.Parameters.AddWithValue(equip[1]);cmd.Parameters.AddWithValue(equip[2]);cmd.Parameters.AddWithValue(equip[3]);return(Convert.ToBoolean(await cmd.ExecuteScalarAsync(ct)),null);}
            case "world_move":
    ...
    static readonly HashSet<string> SupportedRewards=new(StringComparer.OrdinalIgnoreCase){"ChiefExp","copper","lumber","food","iron","new_building","functionId","new_construction","auto_construction_stop","construction_complete"};
