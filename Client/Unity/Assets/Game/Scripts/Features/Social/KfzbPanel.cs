...
namespace CTXD.Client.Features.Social
{
    public sealed class KfzbPanel : MonoBehaviour
    {
        ...
        void Draw()
        {
            ...
            LegacyUiFactory.PixelLabel(_window, "State " + _view.globalState + "   W " + _view.wins + " / L " + _view.losses + (_view.eliminated ? "   ELIMINATED" : ""), 16, TextAnchor.MiddleCenter, _view.eliminated ? new Color(1,.35f,.25f) : Color.white, 120, 55, 600, 30);
            if (_feastOpen)
            ...
            if (_view.match != null)
            {
                ...
                if (match.battleId > 0 && match.state == 1)
                {
                    ...
                    {
                        try { await _api.GetBattleAsync(battle); _status("KFZB battle " + battle + " ready"); }
                        catch (Exception ex) { _status(ex.Message); }
                    });
                }
        ...
        async void Signup()
        {
            try { _view = await _api.SignupKfzbAsync(); _status("KFZB signup complete"); await Refresh(); }
            catch (Exception ex) { _status(ex.Message); }
        }
